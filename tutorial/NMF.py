...
class CustomNMF(NMF):
    def __init__(self, n_components, init='random', beta_loss='KL', solver='mu', max_iter=10000, tol=1e-6,
        ...
        if self.const_k_W is not None: self.start_k_W = self.const_k_W
        self.start_k_H = [c for c in start_k_H if c < n_components] if start_k_H is not None else None
        if self.const_k_H is not None: self.start_k_H = self.const_k_H
...
if __name__ == '__main__':
    ...
    from lib_code.NMF_lib import load_datasets, NMF
    res_dir = Path('../../plots/data_NMF/H-PL-SCLC/beta=F/solver=mu/gene_scores')
    dataset_list = ['H-PL-SCLC']

    counts = True
    print('1')
    data, atlas, datasets, healthy_baseline, gene_names, sample_names, background = \
        load_datasets(dataset_list=dataset_list, counts=counts, background=counts)
    NMF_init = 'random'
    beta = 'KL'
    solver = 'mu'
    # solver = 'cd'
    ...
    #
    t = data
    X = data.loc[data.index[:t]]
    S = background.loc[data.index[:t]]
    X = X.values.T
    S = S.values.T

    print('2')
    nmf_model = NMF(n_components=2, init='random', max_iter=1000,
                    beta_loss='kullback-leibler', solver=solver)
    nmf_model.fit_transform(X, S=S)
    c=2