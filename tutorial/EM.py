...
#
#     atlas_path = "/cs/wetlab/ElaFallik/simulations/EM_runs/Atlas_tests/N=100_M=200_K=2/Atlas.csv"
#     atlas = np.array(pd.read_csv(atlas_path).values[:, 1:dims.K + 1], dtype='float64')
#
#     from Simulation_code.DataGenerator import DataGenerator
#     data_path = Path("/cs/wetlab/ElaFallik/simulations/EM_runs/Atlas_tests/N=100_M=200_K=2/bg=0_var=3/0/train/data/")
#     data = DataGenerator(data_path, dims=dims).generate().data
#
#     param_path = Path("/cs/wetlab/ElaFallik/simulations/EM_runs/Atlas_tests/N=100_M=200_K=2/bg=0_var=3/0/train/sim_test3/")
#     a = np.array(pd.read_csv(param_path/'start_a.csv'), dtype='float64')
#     b = np.array(pd.read_csv(param_path/'start_b.csv'), dtype='float64')
#     W = np.array(pd.read_csv(param_path/'start_W.csv'), dtype='float64')
#     start_params = Params(W=W, a=a, b=b)
#
#     em = EM(data, start_params, hyper_params, dims)
#     # em.run_EM(10)
#     # print(f'train\t{em.ll:.5f}')
#     # em.run_EM(0.1)
#     # print(f'train\t{em.ll:.5f}')
#     em.get_cdf()
#     em.statistics = em._e_step()
#     Ws = [em.params.W]
#     As = [em.params.a]
#     Bs = [em.params.b]
#     lls = [em.ll]
#     for i in range(40):
#         em.run_itr(i)
#         Ws.append(em.params.W)
#         As.append(em.params.a)
#         Bs.append(em.params.b)
#         lls.append(em.ll)
#         print(f'train\t{em.ll:.5f}')
#     c = 2

# regular EM
# -5.58552, -4.69552, -4.02284, -3.63649, -3.41890, -3.28353, -3.18773, -3.11840, -3.06420, -3.01931

#
# def scatter(a, b, log=False, diag=True, ma=0, ax=plt):
#     if log:
#         a = np.log(a+1)
#         b = np.log(b+1)
#     if diag:
...
# def order_W(W1, W2):
#     K = W1.shape[1]
#     corr_mat = np.corrcoef(W1, W2, rowvar=False)[-K:, :K]
#     idx = linear_sum_assignment(corr_mat.T, maximize=True)
#     return W1[:, idx[0]], W2[:, idx[1]]
#
# N_GENES = 17991
# H_norm = (10 ** 5 / 2) * M / N_GENES
# Ws2, As2, Bs2 = [], [], []
# mus2 = []
# for i in range(len(Ws)):
#     p = Params(W=Ws[i], a=As[i], b=Bs[i])
#     p = normalize_params(Params().copy(p), H_norm=H_norm)
#     Ws2.append(p.W)
#     As2.append(p.a)
#     Bs2.append(p.b)
#     mus2.append(p.a / p.b)
...
# idx = mus3[:, :, 0] < mus3[:, :, 1]
# for j in range(N):
#     k = 0 if (idx[:, j].sum() > 0) else 1
#     plt.plot(mus3[:, j, k], linewidth=.5, alpha=.5, c=CLR[k])
# plt.xticks(np.arange(0, T, 5), np.arange(0, T, 5))
# plt.title('H per itr (H[0]>H[1])')
# plt.xlabel('itr')
# plt.savefig(Path('/cs/wetlab/ElaFallik/simulations/EM_runs/Atlas_tests/ll_res')/
#             'H_per_itr_01.png')
# plt.show()
# for j in range(N):
#     k = 1 if (idx[:, j].sum() > 0) else 0
#     plt.plot(mus3[:, j, k], linewidth=.5, alpha=.5, c=CLR[k])
# plt.xticks(np.arange(0, T, 5), np.arange(0, T, 5))
# plt.title('H per itr (H[1]>H[0])')
# plt.xlabel('itr')
# plt.savefig(Path('/cs/wetlab/ElaFallik/simulations/EM_runs/Atlas_tests/ll_res')/
#             'H_per_itr_10.png')
# plt.show()
...
# for i in range(N):
#     for k in range(K):
#         plt.plot(Ws3[:, i, k], linewidth=.5, alpha=.5, c=CLR[k])
# plt.xticks(np.arange(0, T, 5), np.arange(0, T, 5))
# plt.title('W per itr')
# plt.xlabel('itr')
# plt.savefig(Path('/cs/wetlab/ElaFallik/simulations/EM_runs/Atlas_tests/ll_res')/
...
# ssh://elafallik@cb-01.cs.huji.ac.il:22/cs/wetlab/ElaFallik/labvenv/bin/python3.7 -u /cs/wetlab/ElaFallik/simulations/Code/EM.py
# /cs/wetlab/ElaFallik/labvenv/lib/python3.7/site-packages/numba/np/ufunc/parallel.py:365: NumbaWarning: The TBB threading layer requires TBB version 2019.5 or later i.e., TBB_INTERFACE_VERSION >= 11005. Found TBB_INTERFACE_VERSION = 10006. The TBB threading layer is disabled.
#   warnings.warn(problem)
# _e_step func time:	16.18
# _m_step func time:	0.12
# _e_step func time:	4.55
# train	-3.42204
# _m_step func time:	0.11
# _e_step func time:	4.49
# train	-3.04982
# _m_step func time:	0.11
# _e_step func time:	4.46
# train	-2.91471
# _m_step func time:	0.15
# _e_step func time:	4.67
# train	-2.87215
# _m_step func time:	0.11
# _e_step func time:	4.61
# train	-2.85734
# _m_step func time:	0.11
# _e_step func time:	4.52
# train	-2.85085
# _m_step func time:	0.11
# _e_step func time:	4.46
# train	-2.84724
# _m_step func time:	0.11
# _e_step func time:	4.53
# train	-2.84484
# _m_step func time:	0.14
# _e_step func time:	4.62
# train	-2.84305
# _m_step func time:	0.11
# _e_step func time:	4.68
# train	-2.84164
# _m_step func time:	0.11
# _e_step func time:	4.55
# train	-2.84047
